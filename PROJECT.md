...
**白話版**(使用者原話):`LLM` 是腦,`wasmtime` 是頭骨(容器,現在是真的獨立 process,不只是同 process 內的 fresh Store——這樣 Stop 按鈕才能真的 `SIGKILL` 一次失控的 run,不管它卡在哪),`http_get`/`search_web` 是眼睛(唯讀,看得到外面但摸不到),`ssh_exec` 是身體(可選,唯一真的能改變外部世界的能力),`db_exec` 是海馬迴——RAG 引擎,自動運作,LLM 完全碰不到,只有 `daily_maintenance` 這個專屬蒸餾流程能把東西寫進長期記憶(`memory_get`/`memory_set` 那種隨手小抄設計試過,實際查 DB 發現從沒被用過,已拿掉)。
...
│  └────────┬──────────┘   │  notify / chat_send            │ │
│           │ spawn        │  sleep_until / schedule_task 家族│ │
│           │ `kernel      └──────────────┬──────────────────┘ │
│           │  worker`                    │                    │
│           │ child process(非 in-process thread——           │
│           │  這樣 abort 才能真的 SIGKILL 一個卡住的 run)   │
│  ┌────────▼──────────────────────────────▼───────────────┐ │
...
| `POST /api/abort` | 中斷正在跑的這一次——寫 abort flag(讓剛好在 `llm_call` 串流中的那次能乾淨收尾)**同時直接對這次 run 的 `kernel worker` 子行程送 `SIGKILL`**,保證真的停,不管卡在哪個 syscall |
...
| fresh instantiate | 是 | crash-safe、零殘留。**現況更新**:現在是真的獨立 child process(`kernel worker`,見 §4.2/`main.rs`),不只是同一個 kernel process 內的 fresh `Store`——這樣 `POST /api/abort` 才能對這次 run 直接 `SIGKILL`,不管它卡在哪個 syscall 裡;一個 process 內的 thread 沒辦法安全強殺,只能殺整個 kernel |
...
- [x] `PROJECT.md` 同步實際程式碼(2026-07-11 補做):`memory_get`/`memory_set` 已從全文拿掉、
      `http_fetch`→`http_get` 改名已同步、補了 `ebinactl` 現況(不再是「尚未做的 CLI wrapper」)、
      `daily_maintenance` 6 小時循環、streaming、subprocess+SIGKILL abort、write_file 依 trigger
      type 限定範圍、read_file 分頁/`grep_file`、mid-run compact、http_get cache、WAL mode、
      `db_exec` PRAGMA allowlist 那個 bug,§2/§3/§4.2-4.6/§5 都補了
