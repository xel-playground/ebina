...
- Loop:讀狀態 → RAG 檢索(每 turn 都重新做,不是只在開頭,見 §4.3)→ 組 prompt → llm_call → 解析行動 → 執行 → 寫回 → sleep_until。每輪 llm_call 回來的 `input_tokens` 超過 `runtime.in_run_compact_tokens`(預設 10,000——原本 150,000,2026-07-11 調低,理由是每次內部工具呼叫都會把整包 messages 重送一次,門檻設太高等於放任單一 turn 累積出好幾次昂貴的大 context 呼叫)會觸發 run 內自己的 compact——system prompt + 最開頭那則任務訊息永遠保留(避免摘要層層疊加後忘記最初的意圖),中間全部摘要掉,跟 `daily_maintenance`(記憶蒸餾)、`[chat] auto_compact_tokens`(跨 run 的 session 壓縮)是三件不同的事
